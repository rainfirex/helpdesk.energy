...
class LdapInfo {
    ...
    protected
        ...
        //
        $ldapServer = '172.26.79.10.local',
        $ldapDn = "DC=sakh.dvec.ru,DC=local",
        $ldapUserDomain = 'sakh.dvec.ru\\',


        // системный пользователь - для получения информации о любом пользователе без пароля
        $SYS_LOGIN = 'Poplavskiy-AA@sakh.dvec.ru',
        $SYS_PWD   = 'syspassword';


    public function run(string $action, string $login, string $password) {
        $this->LOGIN       = trim($login);
        $this->PWD         = trim($password);
...
//        dd( 'Соединение успешно!', $this->ldapConnected);
    }
