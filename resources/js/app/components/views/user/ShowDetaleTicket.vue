                ...
                <div v-for="(comment, index) in comments" :key="comment.id">
                    ...
                    <div class="comment-box">
                        ...
                        >
                            ...
                            <span class="mb-0 text-muted h6 pt-2 pb-2 comment-about">Ответ:&nbsp;<span class="username">{{comment.user.name}}:</span></span>
                            <p>{{comment.description}}</p>
                        </div>

                        <div class="comment comment-user" v-else>
                            <p class="m-1 date-created">{{formatDateTime(comment.created_at)}}</p>
                            <span class="mb-0 text-muted h6 pt-2 pb-2 comment-about  ml-4">Вы:</span>
                            <p>{{comment.description}}</p>
                        </div>
                    </div>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            getTicket() {
                ...
                axios.get(`/api/user/tickets/id/${ this.ticket_id }/get-ticket`).then(response => {
            ...
            getComments() {
                this.setLoaderBar(true);
                axios.get(`/api/user/comments/ticket-id/${this.ticket_id}/get-comments`).then(response => {
                    this.setLoaderBar(false);
                    if (response.data.success) {
            ...
            getScreenshots() {
                axios.get(`/api/screenshots/ticket-id/${ this.ticket_id }/get-screenshots`).then(response => {
            ...
            getComment(comment_id) {
                this.setLoaderBar(true);
                axios.get(`/api/user/comments/comment-id/${ comment_id }/get-comment`).then(response => {
                    this.setLoaderBar(false);
                    if (response.data.success) {
            ...
            createComment() {
                ...
                this.setLoaderBar(true);
                axios.post(`/api/user/comments/create-comment`, {ticket_id: this.ticket_id, description: this.description.trim()}).then(response => {
                    this.setLoaderBar(false);
            ...
            resetCommentNew(comment) {
                ...
                axios.put(`/api/user/comments/id/${ comment.id }/reset-new`).then(response => {
                    if (response.data.success) {
                        comment.is_new = 0;
                    }
                }).catch(error => {
            ...
            getStatus() {
                this.setLoaderBar(true);
                axios.get(`/api/user/tickets/id/${ this.ticket_id }/status-ticket`).then(response => {
                    this.setLoaderBar(false);
                    if (response.data.success) {
            ...
            completeTicket(){
                ...
                this.setLoaderBar(true);
                axios.put(`/api/user/tickets/completed/ticket-id/${ this.ticket_id }/complete`, {description : this.descriptionComplete}).then(response => {
                    this.setLoaderBar(false);
                    if (response.data.success) {
                        Sound.playSound('/sounds/_notify.mp3');
